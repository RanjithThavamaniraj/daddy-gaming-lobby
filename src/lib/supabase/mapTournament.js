...
export function mapEnrichedTournamentRow(row, resultsRow) {
  ...
  return applyLifecycleStatus({
    ...
    teamMainSize: row.metadata?.team_main_size ?? undefined,
    teamSubstituteSize: row.metadata?.team_substitute_size ?? undefined,
    registrationMode: row.metadata?.registration_mode ?? undefined,
